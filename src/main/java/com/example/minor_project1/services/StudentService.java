...
//    }
    ...
    public GetStudentDetailResponse getStudentDetails(Integer studentId, boolean requireBookList){
        Student student = this.studentRepository.findById(studentId).orElse(null);
        List<Book> bookList = null;
        if(requireBookList){
        ...
        return GetStudentDetailResponse.builder()
                .student(student)
                .bookList(bookList)
                .build();
    }
...
//        return student.getId(); // default value of integer datatype
    ...
    public GetStudentDetailResponse update(Integer studentId, UpdateStudentRequest request) {

        Student student = request.mapToStudent();
        student.setId(1);

        GetStudentDetailResponse studentDetailsResponse = this.getStudentDetails(studentId, false);
...
//   Another way to retrieve the student details: Student savedStudent = this.studentRepository.findById(studentId).orElse(null);
    ...
    public GetStudentDetailResponse deactivate(int studentId) {
